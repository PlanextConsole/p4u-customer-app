...
class ApiSession {
  ...
  Future<void> saveAuth(Map<String, dynamic> data, {Map<String, dynamic>? profile}) async {
    ...
    if (customerId != null) await prefs.setString(_customerIdKey, customerId.toString());
...
List<Map<String, dynamic>> apiItems(Object? value) {
  ...
  if (value is Map) {
    ...
    for (final key in ['items', 'data', 'results', 'rows', 'banners', 'products', 'services', 'categories']) {
